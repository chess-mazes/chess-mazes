...
class GameModel {
    ...
    validateMove(startRow, startCol, endRow, endCol, blackToMove = false) {
        // Check if the move is within the bounds of the board
        if (startRow < 0 || startRow >= GameModel.NumRows ||
            startCol < 0 || startCol >= GameModel.NumCols ||
            endRow < 0 || endRow >= GameModel.NumRows ||
            endCol < 0 || endCol >= GameModel.NumCols) {
        ...
        if ((blackToMove && startColor === PieceColor.White) ||
    ...
    findSquareThreat(row, col, blackToMove = false) {
