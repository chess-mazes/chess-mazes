...
export class GameModel {
  ...
  validateMove(
    ...
    if (pieceType === "p") {
      ...
      const isMovingOneRow = isWhite
        ? endRow === startRow - 1
        : endRow === startRow + 1;
      const isOnStartingRow = isWhite ? startRow === 6 : startRow === 1;
      const isMovingTwoRowsFromStart = isWhite
        ? endRow === startRow - 2
        : endRow === startRow + 2;
      ...
      if (
        this.findSquareThreat(endRow, endCol, startColor === PieceColor.White)
      ) {
    ...
    for (let i = 0; i < this.board.length; i++) {
      ...
      if (piece !== "") {
        ...
        if (pieceColor == (blackToMove ? PieceColor.Black : PieceColor.White)) {
          ...
          let pieceCol = i % 8;
          if (
            this.validateMove(pieceRow, pieceCol, row, col, blackToMove).status
          ) {
